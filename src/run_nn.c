...
int dotprod_hw(int n_in, volatile int *w, volatile int *ifmap)
{
    ...
    *(dotprod_acc + 2) = (unsigned) w; //weight matrix byte address
    *(dotprod_acc + 3) = (unsigned) ifmap;//input activations vector byte address
    *(dotprod_acc + 5) = (unsigned) n_in; //number of input activations
    *dotprod_acc = 0; /* start */
...
void apply_layer_dot(int n_in, int n_out, volatile int *b, volatile int *w, int use_relu, volatile int *ifmap, volatile int *ofmap)
{
    ...
    //n_out <- number of output activations
    //b <- bias vector
    //w <- weight matrix
    //use_relu <- 1 if ReLU activation is to be applied, 0 otherwise
    //ifmap <- input activations vector
    //ofmap <- output activations vector
